...
UI公開型は `ReviewRequest` / `AICommentDraft`。ネイティブ層で Mock、OpenAI API、OpenAI互換Chat Completions API、Codex CLI、Claude Code CLIを交換する。すべて同じ構造化コメント契約をRustとUI双方で検証する。入力はSHA-256ハッシュのみ `ai_runs` に記録し、本文は記録しない。実行中はリクエストIDごとのキャンセル通知でHTTPまたは子プロセスfutureを破棄できる。
...
Tauri capabilityは `core:default` だけで、shell/plugin/http権限は付与しない。ネイティブCLI連携も任意shellを公開せず、固定バイナリと固定引数だけをRustから直接spawnする。CSPは外部URLを許可しない。API通信はRust `reqwest` のみを使う。公式OpenAIは固定URL、互換APIはユーザーが明示的に保存したHTTPSベースURLに限定し、URL内認証情報・クエリ・フラグメントを拒否する。互換APIキーは正規化したベースURLのハッシュ単位でOS資格情報ストアへ分離保存する。MarkdownはHTMLへ展開してDOMへ挿入せず、Tiptap JSONから生成する。書き出し先はアプリデータ配下へ固定し、任意パスは受け取らない。
