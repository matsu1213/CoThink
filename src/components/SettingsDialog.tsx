...
export function SettingsDialog({onClose}: {onClose(): void}) {
  ...
  useEffect(() => {
    if (provider === 'openai' && !current.hasApiKey) return;
  ...
  const changeProvider = (next: AIProviderKind) => {
    setProvider(next); setModel(defaultModels[next]); setModels([]); setMessage(''); setError(undefined);
  };
  const save = async () => {
    try {
  ...
  return <div className="modal"><div className="dialog settings">
    ...
    <label>モデル<div className="model-picker"><input aria-label="モデル名" value={model} onChange={event => setModel(event.target.value)}/><button type="button" disabled={modelsLoading || (provider === 'openai' && !current.hasApiKey)} onClick={() => void loadModels(provider, true)}>{modelsLoading ? '取得中…' : '一覧を更新'}</button><select className="model-options" aria-label="利用可能なモデル一覧" value="" disabled={models.length === 0} onChange={event => event.target.value && setModel(event.target.value)}><option value="">{models.length ? `候補から選択（${models.length}件）` : '利用可能なモデルを取得してください'}</option>{models.map(available => <option key={available} value={available}>{available}</option>)}</select></div><small>入力中のモデル名にかかわらず、取得した候補をすべて表示します。{provider === 'codex_cli' ? ' Codex CLIのログイン中アカウントで利用可能なモデルを取得します。' : provider === 'openai' ? ' 保存済みAPIキーで利用可能なモデルを取得します。' : provider === 'claude_cli' ? ' Claude Codeのモデル名または別名（通常は sonnet）も直接入力できます。' : ''}</small></label>
